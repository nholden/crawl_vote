...
RSpec.describe "votes" do
  ...
  scenario "successfully creating a vote" do
    post votes_path(vote: { user_uuid: user_uuid, crawl_spot_id: crawl_spot.id })
  ...
  scenario "attempting to create a vote without a user_uuid" do
    post votes_path(vote: { crawl_spot_id: crawl_spot.id })

    expect(crawl_spot.votes.count).to eql 0
    expect(response.status).to eql 400
    expect(JSON.parse(response.body)['user_uuid']).to include "can't be blank"
  end

  scenario "attempting to create a vote without a crawl_spot_id" do
    post votes_path(vote: { user_uuid: user_uuid })
